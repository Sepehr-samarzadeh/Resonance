...
struct MatchViewModelTests {
    ...
    func loadMatchesSuccess() async {
        ...
        match.stubbedFetchMatches = .success(testMatches)
    ...
    func loadMatchesFailure() async {
        let match = MockMatchService()
        match.stubbedFetchMatches = .failure(NSError(domain: "test", code: 1, userInfo: [NSLocalizedDescriptionKey: "Network error"]))
    ...
    func loadMatchesLoading() async {
        let match = MockMatchService()
        match.stubbedFetchMatches = .success([])
